...
def offline_label_market_context():
    ...
    df["ema_50_slope"] = df["ema_50"].diff().rolling(5).mean()
    ...
    df["range_expansion"] = (
        df["true_range"] > df["true_range"].rolling(5).mean()
    ).astype(int)
    ...
    df["atr_expanding"] = (
        df["atr_14"] > df["atr_14"].rolling(10).mean()
    ).astype(int)
    ...
    df["momentum_decay"] = (
        df["range_efficiency"] <
        df["range_efficiency"].rolling(10).mean()
    ).astype(int)

    df["candle_overlap"] = (
        df["high"].rolling(5).min() <
        df["low"].rolling(5).max()
    ).astype(int)

    # =========================================================
    # TIME CONTEXT
    # =========================================================
    df["minute_of_day"] = df["ts"].dt.hour * 60 + df["ts"].dt.minute - 555
    ...
    df["is_gap_auction"] &= ~auction_resolved.shift(1).fillna(False)
    ...
    compression = (
        (df["atr_pct"] < df["atr_pct"].rolling(20).mean() * 0.7) &
        (df["bb_width"] < df["bb_width"].rolling(20).mean() * 0.7) &
        (df["range_efficiency"] < 0.30)
    )

    # ================= GAP AUCTION (HIGHEST PRIORITY) =================
    df.loc[df["is_gap_auction"], "market_phase"] = "GAP_AUCTION"

    # ================= BASE REGIMES (ONLY IF UNCLASSIFIED) =================
    df.loc[
        balance_chop & (df["market_phase"] == "UNCLASSIFIED"),
        "market_phase"
    ] = "BALANCE_CHOP"
    ...
    for i in range(1, len(df)):
        ...
        # ---------------- GAP CONTEXT ----------------
        gap_active = (
            df.at[i, "gap_regime"] == "LARGE_GAP" and
            df.at[i, "minute_of_day"] < 180
        )

        # -------- LOCAL IMPULSE PERMISSION (NO MUTATION) --------
        impulse_allowed = True
        if gap_active:
            if df.at[i, "vwap_acceptance"] == 0:
                impulse_allowed = False
            elif df.at[i, "gap_dir"] == "UP" and df.at[i, "vwap_dist_pct"] < 0:
                impulse_allowed = False
            elif df.at[i, "gap_dir"] == "DOWN" and df.at[i, "vwap_dist_pct"] > 0:
                impulse_allowed = False

        # -------- GAP STATE CONTROL --------
        if gap_active and df.at[i, "market_phase"] in ("BALANCE_CHOP", "COMPRESSION"):
            df.at[i, "market_phase"] = "GAP_BALANCE"
        ...
        if df.at[i, "post_impulse_active"] == 1:
            ...
            df.at[i, "market_phase"] = "DIGESTION"
            df.at[i, "post_impulse_story"] = "PAUSE_AFTER_IMPULSE"
            continue
